-- | A simple functional programming language to illustrate the difference
--   between static scoping and dynamic scoping, and among several different
...
-- | Abstract syntax of our simple functional language.
data Exp
...
dsem (Add l r)   m = case (dsem l m, dsem r m) of
                       ...
                       _ -> Nothing
dsem (Let x b e) m = case dsem b m of    -- Let x b e  ===  App (Fun x e) b
                       Just v -> dsem e ((x,v):m)
                       _ -> Nothing
...
--
-- ** Static scoping, call-by-name (similar to hygienic macros)
--
...
--   
exSlow :: Exp
exSlow = Let "times32" (Fun "x"
           (Add (Add (Add (Add (Add (Ref "x") (Ref "x"))
                               (Add (Ref "x") (Ref "x")))
                          (Add (Add (Ref "x") (Ref "x"))
                               (Add (Ref "x") (Ref "x"))))
                ...
                (Add (Add (Add (Add (Ref "x") (Ref "x"))
                     ...
                     (Add (Add (Add (Ref "x") (Ref "x"))
                          ...
                          (Add (Add (Ref "x") (Ref "x"))
                               (Add (Ref "x") (Ref "x")))))))
    $ App (Ref "times32") (App slow (Lit 1))
...
--   corresponds to the Left case in Ref). Then, to remember that we have
