...
--   False
--
-- Check for refs to find if they are in the list of vars
checkExpr :: [Var] -> Expr -> Bool
checkExpr l ex1 = case ex1 of 
          Ref v -> elem v l
          Lit i -> True 
          Add ex1 ex2 -> case ((checkExpr l ex1), (checkExpr l ex2)) of 
                        (True, True)  -> True 
                        _             -> False 
          Mul ex1 ex2 -> case ((checkExpr l ex1), (checkExpr l ex2)) of
                        (True, True)  -> True 
                        _             -> False 
...
checkCmd m v (Pen mo) = True 
checkCmd m v (Move ex1 ex2) = if ((checkExpr v ex1) == True) &&  ((checkExpr v ex2) == True) then True else False 
checkCmd m v (Call ma ar) = case get ma m of 
                            Just i  -> if i == (length ar) then (all (checkExpr v) ar) else False
                            Nothing -> False 
checkCmd m v (For va ex1 ex2 bl) = case ((checkExpr newV ex1), (checkExpr newV ex2), (all (checkCmd m newV) bl)) of
                                    (True, True, True) -> True
                                    _                   -> False
  where 
    newV = checkVar v va
...
checkVar vs v = if (elem v vs) == True then vs else v : vs
...
checkBlock m vs bl = (all (checkCmd m vs) bl)
...
checkDef m (Define ma pa bl) = checkBlock m pa bl
...
--
expr :: Env -> Expr -> Int
expr env ex = case ex of 
          Mul ex1 ex2 -> (expr env ex1) * (expr env ex2)
          Add ex1 ex2 -> (expr env ex1) + (expr env ex2)
          Ref var     -> getOrFail var env
          Lit i       -> i
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Call macro args -> case getOrFail macro defs of
                       ([],[]) -> (state,[])
                       (pars,(bl)) -> block defs (setAll pars (map (expr env) args) env) state bl
